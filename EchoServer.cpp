...
void ClientThreadFunc(Client* client)
{
...
#else
	auto hsByteCount = read(client->Socket, handshake_msg, sizeof(handshake_msg) - 1);
#endif
    ...
    while (active)
	{
...
#else
		int bytesRead = (int)read(Socket, client->Buffer, bufSize);
#endif
		...
		{
            int amount = msgSize - bytesRead;
            if (amount <= 0 || amount > sizeof(client->Buffer))
            {
                amount = sizeof(client->Buffer);
            }
            cout << "Server about to block on read/recv (read size is ";
            cout << amount << ")\n";
            
#if defined(WIN32) || defined(_WIN32)
			int numRead = recv(Socket, client->Buffer, amount, 0);
#else
			int numRead = (int)read(Socket, client->Buffer, amount);
#endif
            
            cout << "Server completed blocking read/recv call (";
            cout << numRead << " bytes read)\n";
            
            if (0 == numRead)
            {
                cout << "Server received 0 bytes -> client disconnect\n";
                active = false;
                break;
            }
			...
			bytesRead += numRead;
			// Append to memory stream
			ms.Write(client->Buffer, numRead);
		}
...
#endif
		// Clear buffer
        cout << "About to clear " << strlen(client->Buffer) << " bytes in buffer\n";
